...
- `GridItem` - Component to render for each property in `grid` view.
- `ListItem` - Component to render for each property in `list` view.
- `defaultView` - The default view to display when no hash is present.
