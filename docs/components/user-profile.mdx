...
const RegisterForm = () => (
  <UserRegisterForm>
    ...
    <UserInput name="password_confirmation" />
  </UserRegisterForm>
);
```
...
Form for logging a user in. See example below for full usage with `SignInInput` components.

## `SignInInput`

```jsx
import { UserSignInForm } from 'homeflowjs/user';
